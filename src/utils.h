 ...
 */
void DrawButton(Font gameFont,string text, int fontSize, int width, int height, int x, int y, void (*callback)() = nullptr)
{
    // draw rectangle using corresponding width and height and x and y
    DrawRectangle(x, y, width, height, GetColor(NORD_BACKGROUND));

    // use x,y to draw text in the center of the rectangle
    DrawTextEx(gameFont,text.c_str(),
             {(float)(x + width / 2 - MeasureText(text.c_str(), fontSize) / 2),
             (float)(y + height / 2 - fontSize / 2)},
             fontSize,0,
             GetColor(NORD_FOREGROUND));
